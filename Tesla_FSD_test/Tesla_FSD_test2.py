...
def generate_video_frames(condition, temp, visibility, current_acc, ir_acc, problem_decision, solution_decision):
    ...
    # Intro frame (3 seconds at 10 fps)
    intro = np.zeros((frame_height, frame_width, 3), dtype=np.uint8)
    cv2.putText(intro, "Adam's FSD WeatherSync: Problem vs Solution", (50, 180), 
                cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    cv2.putText(intro, f"Weather: {condition.capitalize()}, Vis: {visibility} km", (50, 230), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
    for _ in range(30):  # 3 seconds at 10 fps
        frames.append(intro)
    
    # Animation frames (3 seconds at 10 fps)
    for i in range(30):  # 30 frames total for 3 seconds
        frame = np.zeros((frame_height, frame_width, 3), dtype=np.uint8)
        x_shift = 50 + i * 3  # Smoother motion
        
        # Problem side (left: foggy RGB)
        problem_img = np.ones((300, 400), dtype=np.uint8) * 70
        if current_acc > 50:  # Faint if moderate accuracy
            cv2.rectangle(problem_img, (150, 80), (200, 200), (120), -1)  # Brighter pedestrian
            cv2.rectangle(problem_img, (x_shift, 180), (x_shift + 100, 230), (120), -1)  # Tesla
            cv2.rectangle(problem_img, (x_shift + 30, 150), (x_shift + 80, 180), (120), -1)
            cv2.circle(problem_img, (x_shift + 20, 230), 15, (100), -1)
            cv2.circle(problem_img, (x_shift + 80, 230), 15, (100), -1)
        problem_img = random_noise(problem_img, mode="gaussian", var=0.03) * 255  # Less noise
        ...
        if condition in ["fog", "rain"]:
            solution_img = random_noise(solution_img, mode="gaussian", var=0.02) * 255  # Less noise
        solution_img = cv2.applyColorMap(np.clip(solution_img, 0, 255).astype(np.uint8), cv2.COLORMAP_HOT)  # Hotter colormap
        ...
        cv2.putText(solution_img, "Tesla", (x_shift, 140), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.putText(solution_img, "Pedestrian", (150, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
    ...
    # Write video
    out = cv2.VideoWriter("outputs/ir_detection.mp4", cv2.VideoWriter_fourcc(*"mp4v"), 10, (frame_width, frame_height))
    for frame in frames:
        out.write(frame)
    out.release()
...
def run_fsd_weathersync():
    ...
    # Report
    report = (
        f"FSD WeatherSync Simulation (Adam's Project)\n"
